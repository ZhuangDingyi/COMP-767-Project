        ...
        if self.tl_type != "actuated":
            for i in range(self.rows * self.cols):
                self.k.traffic_light.set_state(
                    node_id='center' + str(i), state="GGGggrrrrrGGGggrrrrr")
...
class TrafficLightGridPOEnv(TrafficLightGridEnv):
    ...
    * num_observed: number of vehicles nearest each intersection that is
    ...
    def observation_space(self):
        ...
        state.
        """
        # Found in 2020/03/25 that 3 egdes (ramp,bot0_1,top0_2) actually do not contribute
        '''
    ...
    def get_state(self):
        ...
        obs = np.array(
            np.concatenate([
                ...
                self.direction.flatten().tolist(),
                self.currently_yellow.flatten().tolist()
            ])) # We may need to delete last change 2020/03/26 
        obs[obs>1]=1
        obs[obs<0]=0
        return obs
