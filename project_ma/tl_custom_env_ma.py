...
from tl_custom_env import TrafficLightGridPOEnv
...
class MultiTrafficLightGridPOEnv(TrafficLightGridPOEnv, MultiEnv):
    ...
    def get_state(self):
        ...
        for rl_id in self.k.traffic_light.get_ids():
            #print('*'*10,rl_id)
            #print(np.shape(speeds),np.shape(dist_to_intersec),np.shape(edge_number),np.shape(density),np.shape(velocity_avg),np.shape(direction),np.shape(currently_yellow))
            rl_id_num = int(rl_id.split("center")[ID_IDX])
            local_edges = node_to_edges[rl_id_num][1]
            #print('local edges: ',local_edges)
            local_edge_numbers = [self.k.network.
            get_edge_list().index(e)
            ...
            #print('local id nums : ',local_id_nums)
            ...
            # Found in 2020/03/24 that obs might excess box upper bound
                ...
                if self.last_change[i] >= self.min_switch_time:
                    if self.direction[i] == 0:
                        self.k.traffic_light.set_state(
                            node_id='center{}'.format(i), state="GGGggrrrrrGGGggrrrrr")
                    else:
                        self.k.traffic_light.set_state(
                            node_id='center{}'.format(i), state='rrrrrGGGggrrrrrGGGgg')
                    self.currently_yellow[i] = 0
            else:
                if action:
                    if self.direction[i] == 0:
                        self.k.traffic_light.set_state(
                            node_id='center{}'.format(i), state='yyyyyrrrrryyyyyrrrrr')
                    else:
                        self.k.traffic_light.set_state(
                            node_id='center{}'.format(i), state='rrrrryyyyyrrrrryyyyy')
